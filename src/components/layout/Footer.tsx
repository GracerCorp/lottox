...
export function Footer() {
  ...
  const links = [
    { title: t.footer.worldDashboard, href: "/global_results" },
  ...
  return (
    <footer className="relative w-full overflow-hidden border-t border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-neutral-950 pb-6 pt-10">
      {/* Background noise */}
      <div className="absolute inset-0 bg-[url('/noise.png')] opacity-[0.02] mix-blend-overlay" />

      <div className="container relative z-10 mx-auto px-4">
        {/* Why LOTTOX Section - Compact Version */}
        <div className="flex flex-col items-center justify-between gap-4 rounded-xl border border-gray-200 dark:border-white/5 bg-white dark:bg-neutral-900/50 px-6 py-4 backdrop-blur-sm md:flex-row shadow-sm">
          ...
          <nav className="flex flex-wrap justify-center gap-6 md:justify-start">
            {links.map((link) => (
              <Link
                key={link.title}
                href={link.href}
                className="text-xs font-medium uppercase tracking-wider text-gray-500 hover:text-neutral-900 dark:hover:text-white transition-colors"
              >
                {link.title}
              </Link>
            ))}
